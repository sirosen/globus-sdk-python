...
class ClientCredentialsAuthorizerTests(CapturedIOTestCase):
    ...
    def setUp(self):
        ...
        self.mock_response.other_tokens = []
    ...
    def test_get_token_response(self):
        ...
        ConfidentialAppAuthClient is used and the mock response was returned.
        """
        ...
        self.cac.oauth2_client_credentials_tokens.assert_called_once()

    def test_multiple_resource_servers(self):
        """
        Sets the mock ConfidentialAppAuthClient to return other_tokens
        Confirms GlobusError is raised when _get_token_response is called.
        """
        self.mock_response.other_tokens = [{"access_token": "token"}]

        with self.assertRaises(GlobusError) as err:
            self.authorizer._get_token_response()
